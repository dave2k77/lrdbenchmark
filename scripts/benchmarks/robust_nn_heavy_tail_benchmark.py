#!/usr/bin/env python3
"""
Robust Neural Network Heavy-Tail Benchmark

This script tests how Neural Network estimators perform on pure data versus 
alpha-stable heavy-tailed data using our new robust optimization backend.
...
def test_robust_nn_estimator(estimator, data: np.ndarray, true_hurst: float, name: str, 
                           preprocessor: AdaptiveDataPreprocessor,
                           backend: RobustOptimizationBackend) -> Dict[str, Any]:
    """Test a Neural Network estimator on data using robust preprocessing."""
    try:
        ...
        if len(data_processed) < 10:
            return {
                ...
                'error': np.nan,
                'success': False,
                'reason': 'Insufficient data after preprocessing'
            }
    ...
    return {
        ...
        'original_length': preprocess_metadata.get('original_length', len(data)),
        'processed_length': len(data_processed) if 'data_processed' in locals() else 0
    }

def run_robust_nn_heavy_tail_benchmark():
    """Run the robust Neural Network heavy-tail benchmark."""
    print("🧠 Robust Neural Network Heavy-Tail Benchmark")
    print("=" * 50)
    ...
    for hurst in hurst_values:
        ...
        for length in data_lengths:
            ...
            for est_name, estimator in estimators.items():
                fbm_result = test_robust_nn_estimator(estimator, fbm_data, hurst, f"{est_name}_FBM", 
                                                    preprocessor, backend)
                fgn_result = test_robust_nn_estimator(estimator, fgn_data, hurst, f"{est_name}_FGN", 
                                                    preprocessor, backend)
            ...
            for alpha in alpha_values:
                ...
                for est_name, estimator in estimators.items():
                    alpha_result = test_robust_nn_estimator(estimator, alpha_data, hurst, f"{est_name}_Alpha{alpha}", 
                                                          preprocessor, backend)
...
def create_visualization(results: List[Dict[str, Any]], data_types: List[str], 
    ...
    try:
        ...
        fig.suptitle('Robust Neural Network Estimators: Pure vs Alpha-Stable Data Performance', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('robust_nn_heavy_tail_benchmark.png', dpi=300, bbox_inches='tight')
        print(f"\n📊 Visualization saved as 'robust_nn_heavy_tail_benchmark.png'")
        
    except Exception as e:
        print(f"⚠️  Could not create visualization: {e}")

if __name__ == "__main__":
    print("🚀 Starting Robust Neural Network Heavy-Tail Benchmark...")
    print("   Testing Neural Network estimators with robust preprocessing on pure vs alpha-stable data")
    print("   Focus: Robustness to heavy-tailed noise with improved data preprocessing")
    
    try:
        results = run_robust_nn_heavy_tail_benchmark()
        analyze_results(results)
        
        print(f"\n✅ Robust Neural Network Heavy-Tail Benchmark completed!")
