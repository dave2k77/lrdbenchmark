#!/usr/bin/env python3
"""
Full Benchmarking Experiment Script
===================================

Comprehensive benchmarking experiment for LRD estimators covering:
...
class ExperimentRunner:
    """
    Comprehensive experiment runner for LRD estimator benchmarking.
    ...
    def __init__(self, output_dir: str = "experiment_results", n_realizations: int = 10, 
        ...
        # Get all 13 estimators
        self.estimators = self.benchmark.get_estimators_by_type("comprehensive")
    ...
    def run_pure_experiment(self):
        """
        Run pure synthetic process experiments (n=320 in full mode).
    ...
    def run_contaminated_experiment(self):
        """
        Run contaminated process experiments (n=240 in full mode).
        Base: fBm with H ∈ {0.5, 0.7, 0.9}
        8 contamination types at 10% level
        """
        print("\n" + "="*60)
    ...
    def run_realistic_experiment(self):
        """
        Run realistic scenario experiments (n=300 in full mode).
        15 scenarios × 2 lengths × 10 replications
        """
        print("\n" + "="*60)
    ...
    def compute_statistics(self) -> Dict[str, Any]:
        ...
        if not df_pure.empty:
            try:
                # Pivot table: rows = conditions, columns = estimators
        ...
        if not df_pure.empty and "pure" in stats_results:
            try:
                estimator_mae = df_pure.groupby("Estimator")["AbsError"].mean()
                best_estimator = estimator_mae.idxmin()
                print(f"  Best estimator (lowest MAE): {best_estimator}")
                
                wilcoxon_results = []
                other_estimators = [e for e in estimator_mae.index if e != best_estimator]
                n_comparisons = len(other_estimators)
                alpha = 0.05
                ...
                for est in other_estimators:
                    try:
                        ...
                        pivot = df_pure.pivot_table(
                            ...
                            columns="Estimator",
                            aggfunc="first"
                        )[[best_estimator, est]].dropna()
                        
                        if len(pivot) >= 5:
                            stat, p_value = stats.wilcoxon(
                                pivot[best_estimator].values,
                                pivot[est].values,
                                alternative='less'
                            )
                            wilcoxon_results.append({
                                ...
                                "statistic": float(stat),
                                "p_value": float(p_value),
                                "bonferroni_p": float(p_value * n_comparisons),
                                "significant": p_value < bonferroni_alpha
                            })
                    except Exception:
                        pass
                
                stats_results["wilcoxon_tests"] = {
                    "reference": best_estimator,
    ...
    def generate_report(self):
        ...
        if not df_pure.empty:
            ...
            plt.ylabel("RMSE")
            plt.title("Estimator Performance on Pure Synthetic Data (RMSE)")
        ...
        if not df_real.empty:
            ...
            domain_pivot.plot(kind="bar", figsize=(16, 8), width=0.8)
    ...
    def run_full_experiment(self):
        ...
        print("\n" + "="*60)
        print("LRD ESTIMATOR BENCHMARKING EXPERIMENT")
        print("="*60)
        ...
        print(f"  - Pure: {len(self.pure_results)}")
        print(f"  - Contaminated: {len(self.contaminated_results)}")
        print(f"  - Realistic: {len(self.realistic_results)}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Run full LRD benchmarking experiment")
    parser.add_argument("--n_realizations", "-n", type=int, default=10, 
                       help="Number of realizations per condition (default: 10)")
    parser.add_argument("--output", "-o", type=str, default="benchmark_results_full", 
