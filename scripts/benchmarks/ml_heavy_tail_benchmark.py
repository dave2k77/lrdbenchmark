...
heavy-tailed data to demonstrate their robustness to extreme values.
...
def test_ml_estimator(estimator, data: np.ndarray, true_hurst: float, name: str) -> Dict[str, Any]:
    """Test an ML estimator on data."""
    try:
...
def run_ml_heavy_tail_benchmark():
    """Run the ML heavy-tail benchmark."""
    print("🧠 ML Heavy-Tail Benchmark")
    print("=" * 50)
    ...
    for hurst in hurst_values:
        ...
        for length in data_lengths:
            ...
            for est_name, estimator in estimators.items():
                fbm_result = test_ml_estimator(estimator, fbm_data, hurst, f"{est_name}_FBM")
                fgn_result = test_ml_estimator(estimator, fgn_data, hurst, f"{est_name}_FGN")
            ...
            for alpha in alpha_values:
                ...
                for est_name, estimator in estimators.items():
                    alpha_result = test_ml_estimator(estimator, alpha_data, hurst, f"{est_name}_Alpha{alpha}")
...
def create_visualization(results: List[Dict[str, Any]], data_types: List[str], 
    ...
    try:
        ...
        fig.suptitle('ML Estimators: Pure vs Alpha-Stable Data Performance', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('ml_heavy_tail_benchmark.png', dpi=300, bbox_inches='tight')
        print(f"\n📊 Visualization saved as 'ml_heavy_tail_benchmark.png'")
        
    except Exception as e:
        print(f"⚠️  Could not create visualization: {e}")

if __name__ == "__main__":
    print("🚀 Starting ML Heavy-Tail Benchmark...")
    print("   Testing ML estimators on pure vs alpha-stable data")
    print("   Focus: Robustness to heavy-tailed noise")
    
    try:
        results = run_ml_heavy_tail_benchmark()
        analyze_results(results)
        
        print(f"\n✅ ML Heavy-Tail Benchmark completed!")
