#!/usr/bin/env python3
"""
Standalone test for the GHE (Generalized Hurst Exponent) Estimator.
This test runs the GHE estimator directly without importing the full package.
"""
...
# Import the GHE estimator directly
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'lrdbenchmark', 'analysis', 'temporal', 'ghe'))
...
def test_backend_selection():
    ...
    for name, use_jax, use_numba in backends:
        print(f"Testing backend preference: {name}")
        try:
            ghe = GHEEstimator(
                q_values=[1, 2, 3],
                tau_min=2,
            ...
            results = ghe.estimate(data)
            backend_used = results['backend_used']
            hurst = results['hurst_parameter']
            
            print(f"   Backend used: {backend_used}")
            print(f"   Hurst estimate: {hurst:.4f}")
