...
The overall success rate was 100\% for ML and Neural Network estimators, and 93.8\% for Classical estimators (150/160 tests) after implementing improved data preprocessing, demonstrating robust performance under various conditions including extreme heavy-tail data. Our comprehensive leaderboard reveals Neural Networks dominate with CNN achieving the highest overall score of 9.66/10 (0.101 MAE), followed by LSTM (9.65/10, 0.104 MAE), GRU (9.63/10, 0.111 MAE), and Transformer (9.62/10, 0.115 MAE). Classical R/S estimator achieves excellent performance at 9.25/10 (0.150 MAE), demonstrating the continued relevance of classical methods.
...
Machine learning estimators demonstrated excellent performance on heavy-tail data with a mean error of 0.219, showing strong robustness across all scenarios:
...
Neural networks achieved excellent performance (0.236 mean error) with 100\% success rates:
...
Surprisingly, classical methods actually performed better on heavy-tailed data (0.277 MAE) compared to pure data (0.305 MAE), suggesting inherent robustness to heavy tails. Failures occurred only on extreme cases with insufficient valid data.
