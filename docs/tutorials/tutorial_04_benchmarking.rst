...
This notebook demonstrates the full benchmarking system of LRDBenchmark,
showing how to systematically compare estimators across different data
types, contamination scenarios, statistical diagnostics, and robustness
stress tests.
...
2. **Contamination Testing**: Evaluate robustness under various
   contamination scenarios
3. **Performance Metrics**: Measure accuracy, speed, and reliability
4. **Statistical Analysis**: Generate confidence intervals, significance
   tests, and stratified summaries
5. **Robustness Panels**: Run automated scaling diagnostics plus stress
   panels for missingness, regime shifts, bursts, and seasonal drift
6. **Visualization**: Create publication-ready plots, stratified tables,
   and robustness figures

Table of Contents
-----------------

1. `Setup and Imports <#setup>`__
2. `Benchmark System Overview <#overview>`__
3. `Classical Estimator Benchmarking <#classical>`__
4. `ML and Neural Network Benchmarking <#ml-neural>`__
5. `Contamination Robustness Testing <#contamination>`__
6. `Performance Analysis <#performance>`__
7. `Results Visualization <#visualization>`__
8. `Summary and Next Steps <#summary>`__
...
First, let’s import all necessary libraries and set up the comprehensive
benchmarking system.
...
.. code:: ipython3
    ...
    initialise_global_rng(1729)  # Ensures reproducible benchmarks
    
    # GPU Memory Management Functions

.. _overview:

2. Benchmark System Overview
----------------------------

The ComprehensiveBenchmark class provides a unified interface for
testing all estimator categories. Let’s examine its capabilities and run
our first benchmark.
...
3. Contamination Robustness Testing
-----------------------------------

Real-world data often contains contamination that can affect estimator
performance. Let’s test how different estimators handle various
contamination scenarios.
...
1. **Comprehensive Benchmarking**: LRDBenchmark provides a unified
   system for testing all estimator categories:
...
2. **Contamination Testing**: The system evaluates robustness under
...
-  **Classical Estimators**: Fast and reliable for standard LRD
   processes
-  **ML Estimators**: Good balance of accuracy and robustness
-  **Neural Networks**: Highest accuracy for complex patterns
-  **Contamination Effects**: Different estimators show varying
   robustness

Next Steps
~~~~~~~~~~

1. **Leaderboard Generation**: Create performance rankings and
   comparisons
2. **Real-world Application**: Apply to actual time series data
3. **Advanced Analysis**: Explore statistical significance and
...
-  ``outputs/contamination_robustness.png``: Contamination effects
   visualization
...
**Next Notebook**:
`05_leaderboard_generation.ipynb <05_leaderboard_generation.ipynb>`__ -
Learn how to create performance leaderboards and rankings.

Stratified and Robustness Diagnostics
-------------------------------------

-  Access ``summary["stratified_metrics"]`` to review error, coverage,
   and confidence-width summaries across H regimes, tail classes, data
   lengths, and contamination levels.
-  For advanced runs, inspect the ``scaling_diagnostics`` and
   ``robustness_panel`` entries returned per estimator to quantify
   scaling influence and stress-test drift.
-  Use ``AnalyticsDashboard().create_advanced_diagnostics_visuals(...)``
   to generate the scaling slope and robustness panel figures for slide
   decks and manuscripts.
