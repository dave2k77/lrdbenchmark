#!/usr/bin/env python3
"""
Comprehensive Benchmark with Neural Networks: All Working Estimators
...
class ComprehensiveBenchmarkWithNN:
    """
    Comprehensive benchmark including neural networks for Hurst parameter estimation.
    """
    
    def __init__(self, output_dir: str = "comprehensive_nn_results"):
    ...
    def _initialize_estimators(self) -> Dict[str, Any]:
        """Initialize all estimators including neural networks."""
        estimators = {}
        ...
        try:
            ...
            nn_networks = create_all_benchmark_networks(input_length=max_length)
            
            # Add neural networks to estimators
            for name, network in nn_networks.items():
                estimators[f"NN_{name}"] = network
                
            logger.info(f"Successfully initialized {len(nn_networks)} neural network architectures")
            ...
            for length in self.data_lengths:
                for sample_idx in range(self.n_samples_per_condition):
                    ...
                    try:
                        ...
                        # Set timeout for this test (30 seconds)
                        ...
                        try:
                            ...
                            if name.startswith("NN_"):
                                ...
                                # Make prediction
                                result = estimator.predict(data.reshape(1, -1))
    ...
    def run_benchmark(self):
        ...
        logger.info("Starting comprehensive benchmark with neural networks...")
        start_time = time.time()
    ...
    def _save_results(self):
        ...
        results_file = self.output_dir / f"comprehensive_nn_benchmark_{timestamp}.json"
        with open(results_file, 'w') as f:
            json.dump(self.results, f, indent=2, default=str)
        
        # Save summary
        summary_file = self.output_dir / f"comprehensive_nn_benchmark_{timestamp}_summary.json"
        with open(summary_file, 'w') as f:
            json.dump(self.summary_df.to_dict('records'), f, indent=2, default=str)
        
        # Save CSV
        csv_file = self.output_dir / f"comprehensive_nn_benchmark_{timestamp}.csv"
...
def main():
    """Run the comprehensive benchmark."""
    benchmark = ComprehensiveBenchmarkWithNN()
